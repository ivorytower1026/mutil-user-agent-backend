...
from src.agent_skills.skill_command_history import (
    get_command_history,
    extract_dependencies_from_commands,
)
...
class ValidationOrchestrator:
    ...
    async def validate_skill(self, skill_id: str) -> dict:
        ...
        async with self.validation_lock:
            with SessionLocal() as db:
                ...
                logger.info(
                    f"[validate_skill] 获取到Skill name={skill.name}, status={skill.status}, path={skill.skill_path}"
                )
                ...
                if existing_state is not None:
                    logger.info(
                        f"[validate_skill] 检测到未完成的验证会话，尝试恢复 thread_id={thread_id}"
                    )
                    result = await self._validate_layer1(skill, config, resume=True)
                    logger.info(
                        f"[validate_skill] Layer1恢复验证完成, passed={result.get('passed')}"
                    )

                    if result.get("passed"):
                        logger.info(
                            f"[validate_skill] 恢复验证通过，更新结果到数据库..."
                        )
                    ...
                    if checkpointer:
                        try:
                            ...
                            logger.info(
                                f"[validate_skill] 已清理checkpoint thread_id={thread_id}"
                            )
                ...
                if skill.status == STATUS_VALIDATING:
                    logger.error(
                        f"[validate_skill] 状态为validating但无checkpoint，标记失败"
                    )
                    self.skill_manager.set_validation_failed(db, skill_id)
                    raise ValueError(
                        f"Validation checkpoint lost for skill {skill_id}, please retry validation"
                    )
                ...
                try:
                    ...
                    logger.info(
                        f"[validate_skill] Layer1验证完成, passed={result.get('passed')}"
                    )
                    ...
                    if not result.get("passed"):
                        ...
                        if checkpointer:
                            try:
                                ...
                                logger.warning(
                                    f"[validate_skill] 清理checkpoint失败: {e}"
                                )
                        return result
                    ...
                    logger.info(
                        f"[validate_skill] Layer2回归验证完成, passed={layer2_result.get('passed')}"
                    )

                    if layer2_result.get("passed"):
                        logger.info(
                            f"[validate_skill] 两层验证全部通过，标记为completed"
                        )
                        skill = self.skill_manager.update_validation_result(
                            db,
                            skill_id,
                        ...
                        logger.info(
                            f"[validate_skill] 验证结果已保存 scores={result.get('scores')}"
                        )
                    else:
                        logger.warning(
                            f"[validate_skill] Layer2回归验证失败，标记为failed"
                        )
                        self.skill_manager.set_validation_failed(db, skill_id)
                    ...
                    logger.info(
                        f"[validate_skill] 验证流程结束 skill_id={skill_id}, passed={final_passed}"
                    )
                    return {
                        "passed": final_passed,
                        "layer1_report": result.get("layer1_report"),
                    ...
                    logger.error(
                        f"[validate_skill] 验证异常: {e}\n{traceback.format_exc()}"
                    )
                    self.skill_manager.set_validation_failed(db, skill_id)
                    raise e

    async def _validate_layer1(
        self, skill, config: RunnableConfig | None = None, resume: bool = False
    ) -> dict:
        ...
        logger.info(
            f"[_validate_layer1] 开始{mode}验证 skill_id={skill.skill_id}, name={skill.name}"
        )
        ...
        backend = DockerSandboxBackend(
            user_id=f"validation_{skill.skill_id}", workspace_dir=workspace_dir
        )
        ...
        try:
            ...
            collect_task = asyncio.create_task(
                metrics_collector.start_collecting(interval=1.0)
            )
            ...
            if resume:
                ...
                validation_result = await self._run_validation_agent(
                    backend, skill, config
                )

            logger.info(
                f"[_validate_layer1] 验证Agent执行完成, task_evaluations数量: {len(validation_result.get('task_evaluations', []))}"
            )
            ...
            logger.info(
                f"[_validate_layer1] 离线测试完成 passed={offline_result.get('passed')}, blocked_network_calls={offline_result.get('blocked_network_calls')}"
            )
            ...
            logger.info(
                f"[_validate_layer1] 指标收集完成: cpu={metrics.get('cpu_percent')}%, memory={metrics.get('memory_mb')}MB, time={metrics.get('execution_time_sec')}s"
            )
            ...
            offline_score = calculate_offline_score(
                offline_result.get("blocked_network_calls", 0)
            )
            resource_score = calculate_resource_score(metrics)

            logger.info(
                f"[_validate_layer1] 评分计算完成: completion={completion_score}, trigger={trigger_score}, offline={offline_score}, resource={resource_score}"
            )
            ...
            logger.info(
                f"[_validate_layer1] Layer1报告生成完成 passed={passed}, overall_score={scores['overall']}"
            )
    ...
    async def _run_validation_agent(
        self, backend: DockerSandboxBackend, skill, config: RunnableConfig | None = None
    ) -> dict:
        ...
        if skill_md_path.exists():
            with open(skill_md_path, encoding="utf-8") as f:
                skill_md = f.read()
            logger.info(
                f"[_run_validation_agent] 读取SKILL.md成功, 长度: {len(skill_md)}"
            )
        else:
        ...
        logger.info(
            f"[_run_validation_agent] 获取已批准skills数量: {len(approved_skills)}"
        )
...
"""
        ...
        if os.path.exists(skills_dir):
            skills_mounts[_to_docker_path(skills_dir)] = {
                "bind": "/skills",
                "mode": "ro",
            }
        ...
        checkpointer = self.checkpointer
        logger.info(
            f"[_run_validation_agent] 创建DeepAgent checkpointer={'有' if checkpointer else '无'}"
        )
        with SessionLocal() as db:
            big_llm = get_llm_manager().get_big_llm(db)
        agent = create_deep_agent(
        ...
        result = await agent.ainvoke(
            {"messages": [("user", prompt)]}, config=invoke_config
        )
        logger.info(f"[_run_validation_agent] Agent执行完成")

        parsed_result = self._parse_validation_result(result)
        logger.info(
            f"[_run_validation_agent] 结果解析完成 task_evaluations={len(parsed_result.get('task_evaluations', []))}"
        )
    ...
    def _parse_validation_result(self, result) -> dict:
        ...
        logger.info(
            f"[_parse_validation_result] 解析完成 task_evaluations={len(task_evaluations)}, assessment_summary={assessment.get('summary', '')[:50]}"
        )

        return {"task_evaluations": task_evaluations, "assessment": assessment}
    ...
    async def _run_offline_test(self, backend: DockerSandboxBackend, skill) -> dict:
        ...
        try:
            test_result = backend.execute(
                "curl -s --connect-timeout 2 http://google.com 2>&1 || echo 'NETWORK_BLOCKED'"
            )
            output = (
                test_result.output
        ...
        logger.info(
            f"[_run_offline_test] 离线测试完成 passed={passed}, blocked_network_calls={blocked_network_calls}"
        )
    ...
    def _get_approved_skills_paths(self) -> list[str]:
        ...
        with SessionLocal() as db:
            ...
            logger.info(
                f"[_get_approved_skills_paths] 获取已批准skills路径数量: {len(paths)}"
            )
            return paths
    ...
    async def _validate_layer2(self, skill, base_image: str | None = None) -> dict:
        ...
        logger.info(
            f"[_validate_layer2] 将并行测试 {len(approved_skills)} 个已入库 skill"
        )
        ...
        async def test_with_semaphore(approved_skill):
            async with semaphore:
                return await self._test_single_skill_regression(
                    approved_skill, base_image
                )
        ...
        for approved_skill, result in zip(approved_skills, results):
            ...
            if isinstance(result, Exception):
                ...
                logger.error(
                    f"[_validate_layer2] 回归测试异常 skill={skill_name}: {result}"
                )
            else:
                ...
                if not result["passed"]:
                    failed_skills.append(skill_name)
                    logger.warning(
                        f"[_validate_layer2] 回归测试失败 skill={skill_name}"
                    )
                else:
                    logger.info(
                        f"[_validate_layer2] 回归测试通过 skill={skill_name} score={result.get('score')}"
                    )

        passed = len(failed_skills) == 0
        logger.info(
            f"[_validate_layer2] 回归测试完成 passed={passed} failed_count={len(failed_skills)}"
        )
    ...
    async def _test_single_skill_regression(
        self, approved_skill, base_image: str | None = None
    ) -> dict:
        """单个 skill 的回归测试（独立容器）
        ...
        backend = DockerSandboxBackend(
            user_id=f"regression_{skill_name}", workspace_dir=workspace_dir
        )
...
""",
            ...
            try:
                ...
                logger.warning(
                    f"[_test_single_skill_regression] JSON解析失败 skill={skill_name}: {e}"
                )
                return {
                    "passed": True,
                    "score": 60,
            ...
            logger.error(
                f"[_test_single_skill_regression] 异常 skill={skill_name}: {e}"
            )
            return {
                "passed": False,
                "score": None,
            ...
            backend.destroy()
            logger.info(
                f"[_test_single_skill_regression] 容器已销毁 skill={skill_name}"
            )
    ...
    async def resume_all_pending(self) -> int:
        ...
        with SessionLocal() as db:
            ...
            logger.info(
                f"[resume_all_pending] 发现 {len(pending_skills)} 个未完成的验证"
            )
            ...
            for skill in pending_skills:
                ...
                try:
                    ...
                    if existing_state is not None:
                        logger.info(
                            f"[resume_all_pending] 恢复验证 skill_id={skill.skill_id}"
                        )
                        asyncio.create_task(
                            self._resume_validation_task(skill.skill_id, config)
                        )
                        resumed_count += 1
                    else:
                        logger.warning(
                            f"[resume_all_pending] skill_id={skill.skill_id} 无checkpoint，标记失败"
                        )
                        self.skill_manager.set_validation_failed(db, skill.skill_id)
                        db.commit()
                except Exception as e:
                    logger.error(
                        f"[resume_all_pending] 检查失败 skill_id={skill.skill_id}: {e}"
                    )
    ...
    async def _resume_validation_task(self, skill_id: str, config: RunnableConfig):
        ...
        with SessionLocal() as db:
            ...
            try:
                ...
                if result.get("passed"):
                    logger.info(
                        f"[_resume_validation_task] 恢复验证通过 skill_id={skill_id}"
                    )
                    ...
                    logger.warning(
                        f"[_resume_validation_task] 恢复验证未通过 skill_id={skill_id}"
                    )
                    self.skill_manager.set_validation_failed(db, skill_id)
                ...
                if self.checkpointer:
                    try:
                        ...
                        logger.info(
                            f"[_resume_validation_task] 已清理checkpoint skill_id={skill_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"[_resume_validation_task] 清理checkpoint失败: {e}"
                        )

            except Exception as e:
                logger.error(
                    f"[_resume_validation_task] 恢复失败 skill_id={skill_id}: {e}\n{traceback.format_exc()}"
                )
                self.skill_manager.set_validation_failed(db, skill_id)
                db.commit()
