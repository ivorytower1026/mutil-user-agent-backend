...
| 能力 | 实现位置 | 说明 | 复用方式 |
|------|----------|------|----------|
| Docker 沙箱容器管理 | `src/docker_sandbox.py` | 容器创建、执行命令、销毁 | **扩展添加网络控制方法** |
| `/skills` 目录只读挂载 | `src/docker_sandbox.py:177-178` | 全局 Skill 共享 | 直接使用 |
| DeepAgents 框架集成 | `src/agent_manager.py` | `create_deep_agent` 创建 Agent | **直接复用创建验证/执行 Agent** |
| 用户认证 + JWT | `src/auth.py` | Token 验证 | 扩展添加 `is_admin` 字段 |
| Agent 流式对话 | `src/agent_manager.py` | DeepAgents + LangGraph | 直接使用 |
| 分块上传机制 | `src/chunk_upload.py` | 支持大文件上传 | 可复用上传逻辑 |
...
| 来源 | 验证方式 | 自动化程度 | 本方案复用 |
|------|----------|------------|-----------|
| Anthropic Skills API | frontmatter 格式检查 | 低 | 兼容格式规范 |
| OpenAI Skills API | 格式验证 | 低 | - |
| **agentskills.io 规范** | 结构规范定义 | 低 | **兼容 name/description 约束** |
| npm/yarn 包管理 | 依赖解析 + 测试脚本 | 中 | - |
| Docker Hub | 镜像构建 + 自动测试 | 中 | 参考 commit 流程 |
| **python-frontmatter** | YAML 解析库 | - | **直接使用，不自建** |
...
| Skill 名称唯一 | 不允许重名 |
| Docker 沙箱隔离 | 安全性由容器隔离保证，不检测脚本内容 |
