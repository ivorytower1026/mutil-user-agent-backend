...
│ 7. 【阶段1】联网安装依赖                                         │
│ ─────────────────────────────────────────────────────────────── │
│ 验证 Agent 分析 SKILL.md 和文件结构，判断所需依赖：               │
│                                                                 │
│ - pip_packages: ["pandas", "numpy"]                            │
│ - npm_packages: ["lodash"]                                     │
│ - system_packages: ["ffmpeg"]                                  │
│ - scripts: ["curl -o data.zip http://..."]                     │
│                                                                 │
│ 执行安装：                                                       │
...
```python
prompt = f"""
分析以下 Skill 的依赖需求，返回需要安装的依赖：

Skill 内容：
{skill_content}

文件结构：
{list_files(skill_path)}

返回 JSON 格式：
{{
    "pip_packages": ["package1", "package2"],
    "npm_packages": ["package1"],
    "system_packages": ["package1"],
    "scripts": ["curl -o data.zip http://example.com/data.zip"]
}}
"""
```
...
```python
def evaluate_results(skill_md, tasks, execution_results, metrics):
    # 1. 盲测评估：Agent 是否选择了正确的 skill
    trigger_accuracy = sum(r["correct_skill_used"] for r in execution_results) / len(tasks)
    
    # 2. 任务完成度
    completion_rate = sum(r["completed"] for r in execution_results) / len(tasks)
    
    # 3. 内网可用性
    offline_capable = all(r["network_calls"] == 0 for r in execution_results)
...
```python
async def analyze_dependencies(skill_path: str, skill_md: str) -> dict:
    prompt = f"""
分析以下 Skill 的依赖需求：
...
```python
async def install_dependencies(container, dependencies: dict) -> dict:
    results = {}
    ...
    if dependencies.get("system_packages"):
        ...
        results["apt"] = container.exec_run(cmd)
    
    # 4. 执行脚本
    if dependencies.get("scripts"):
        results["scripts"] = []
        for script in dependencies["scripts"]:
            results["scripts"].append(container.exec_run(script))
...
installed_dependencies = {
    "pip": {"pandas": "2.0.0", "numpy": "1.24.0"},
    "npm": {"lodash": "4.17.21"},
    "system": {"ffmpeg": "4.4.2"},
    "installed_at": "2026-02-16T10:00:00Z"
}
...
class DualContainerValidator:
    ...
    def __init__(self, skill_id: str, skill_path: str):
        ...
        self.container1: Optional[docker.models.containers.Container] = None
        self.container2: Optional[docker.models.containers.Container] = None
    ...
    async def _create_container1(self):
        """创建容器1（联网环境）"""
        volumes = {
            str(Path(settings.SHARED_DIR) / "skills"): {
                "bind": "/skills", "mode": "ro"
            },
            self.skill_path: {
                "bind": "/skill_under_test", "mode": "ro"
            },
            str(Path(settings.WORKSPACE_ROOT) / f"validation_{self.skill_id}"): {
                "bind": "/workspace", "mode": "rw"
            }
        }
        
        self.container1 = self.client.containers.create(
            image=self.base_image,
            command="sleep infinity",
            working_dir="/workspace",
            volumes=volumes
            # 不指定 network_mode，使用默认 bridge 网络
        )
        self.container1.start()
        print(f"[DualContainerValidator] Created container1: {self.container1.id}")
    
    async def _commit_temp_image(self) -> str:
        """Commit 容器1为临时镜像"""
        temp_tag = f"temp-{self.skill_id}"
        
        self.container1.commit(
    ...
    async def _create_container2(self):
        ...
        self.container2 = self.client.containers.create(
            image=self.temp_image_id,
            command="sleep infinity",
            working_dir="/workspace",
            volumes=volumes,
            network_mode="none"  # 关键：完全离线
        )
        self.container2.start()
        print(f"[DualContainerValidator] Created container2 (offline): {self.container2.id}")
    
    async def approve_and_lock_image(self, new_version: str) -> str:
        """审核通过后，将临时镜像锁定为正式版本"""
        
        # 重命名临时镜像
        temp_image = self.client.images.get(self.temp_image_id)
        temp_image.tag(settings.SKILL_RUNTIME_IMAGE_PREFIX, tag=new_version)
        
        # 删除临时标签
        self.client.images.remove(self.temp_image_id, force=True)
    ...
    def _cleanup(self):
        ...
        if self.container1:
            try:
                self.container1.remove(force=True)
            except Exception as e:
                print(f"[WARN] Failed to remove container1: {e}")
        
        if self.container2:
            try:
                self.container2.remove(force=True)
            except Exception as e:
                print(f"[WARN] Failed to remove container2: {e}")
...
**解决方案**：使用 Docker SDK 动态控制网络连接

```
┌─────────────────────────────────────────────────────────────────┐
│ 阶段 1：联网安装依赖                                             │
│ ─────────────────────────────────────────────────────────────── │
│ - 创建容器（网络正常，默认 bridge 网络）                          │
...
class NetworkController:
    ...
    def disconnect(self, container_id: str) -> bool:
        """断开容器网络"""
        try:
            container = self.client.containers.get(container_id)
            # 断开默认 bridge 网络
            self.client.networks.get("bridge").disconnect(container)
            print(f"[NetworkController] Disconnected container {container_id}")
            return True
        except docker.errors.APIError as e:
            print(f"[ERROR] Failed to disconnect network: {e}")
            return False
    ...
    def reconnect(self, container_id: str) -> bool:
        ...
        try:
            ...
            print(f"[ERROR] Failed to reconnect network: {e}")
            return False
...
class SkillValidationContainer:
    ...
    def create(self) -> docker.models.containers.Container:
        ...
        self.container = self.client.containers.create(
            ...
            working_dir="/workspace",
            volumes=volumes
            # 注意：不指定 network_mode，使用默认 bridge 网络
        )
        self.container.start()
        print(f"[ValidationContainer] Created container {self.container.id}")
    ...
    async def install_dependencies(self, dependencies: dict) -> dict:
        ...
        if dependencies.get("scripts"):
            results["scripts"] = []
            for script in dependencies["scripts"]:
                exit_code, output = self.container.exec_run(script)
    ...
    def disconnect_network(self) -> bool:
        """断开网络（进入离线模式）"""
        return self.network_controller.disconnect(self.container.id)
    ...
    def destroy(self):
        ...
        if self.container:
            try:
                ...
                print(f"[ValidationContainer] Destroyed container {self.container.id}")
            except Exception as e:
                print(f"[ERROR] Failed to destroy container: {e}")
...
async def validate_skill_with_network_control(skill_id: str, skill_path: str):
    ...
    try:
        ...
        # 1. 创建容器（网络正常）
        container = validation_container.create()
        ...
        # 5. 执行盲测任务
        task_results = await run_blind_tests(container)
...
```python
def disconnect_network(container_id: str):
    """断开容器网络"""
    client = docker.from_env()
    ...
    try:
        ...
        print(f"[WARN] Failed to disconnect network: {e}")
```

### 6.3 网络调用检测

```python
def detect_network_calls(container) -> int:
    """检测被阻止的网络调用次数"""
...
class DependencyValidator:
    ...
    def get_installed_packages(self) -> dict:
        ...
        # pip 包
        _, pip_output = self.container.exec_run("pip freeze")
        pip_packages = self._parse_pip_freeze(pip_output.decode('utf-8'))
        
        # 系统包
        _, dpkg_output = self.container.exec_run("dpkg -l | tail -n +6 | awk '{print $2}'")
        system_packages = dpkg_output.decode('utf-8').strip().split('\n')
...
# 使用示例
async def install_and_validate(container, skill_deps: dict) -> dict:
    """安装依赖并校验"""
