...
| **frontmatter 解析** | `python-frontmatter` | pip 安装，直接使用 | 第三方库 |
| **格式验证规范** | agentskills.io 规范 | 兼容 name/description 约束 | 开放标准 |

### 0.3 需要新增的代码

| 模块 | 新增原因 | 文件 |
|------|----------|------|
| **Skill 验证器** | 业界无运行时验证方案 | `src/skill_validator.py` |
| **双 Agent 验证** | 首创盲测机制 | `src/skill_agent_validator.py` |
| **镜像版本管理** | skill 专用版本控制 | `src/skill_image_manager.py` |
| **Admin API** | 管理接口 | `api/admin.py` |

### 0.4 DockerSandboxBackend 扩展
...
```python
# src/skill_validator.py - 使用 python-frontmatter 库
import frontmatter
import re
import os
...
class SkillValidator:
    """Skill 格式验证器（兼容 agentskills.io 规范）
    ...
    def validate(self, skill_path: str) -> ValidationResult:
        ...
        # 2. 使用 python-frontmatter 解析（复用现有库）
        try:
            ...
            return ValidationResult(
                passed=False,
                errors=[f"Invalid frontmatter: {e}"],
                warnings=[]
            )
        ...
        # 3. 验证 name（兼容 agentskills.io 规范）
        name = skill.get('name', '')
        if not re.match(r'^[a-z0-9]([a-z0-9-]*[a-z0-9])?$', name):
            errors.append("name must be lowercase with hyphens, 1-64 chars")
        if len(name) > 64:
            errors.append("name must be <= 64 chars")
        
        # 4. 验证 description（兼容 agentskills.io 规范）
...
python-frontmatter = "^1.1.0"  # frontmatter 解析
```
