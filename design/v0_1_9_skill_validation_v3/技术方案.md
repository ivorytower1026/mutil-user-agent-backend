...
**子代理**：DeepAgents 内置的 general-purpose 子代理

**执行方式**：串行执行（第一个任务安装依赖较慢，后续较快）
...
validation_agent = create_deep_agent(
    ...
    skills=[skill_under_test] + approved_skills,  # 可访问所有 skill
)

# Agent 自动：
# 1. 使用 write_todo 生成 3 个任务
# 2. 使用 task() 工具串行委托给子代理
# 3. 接收子代理结果
# 4. 自己评分
# 5. 生成评估报告
```
...
| 维度 | 评分方式 | 执行者 | 说明 |
|------|----------|--------|------|
| **任务完成度** | LLM 评估 | 验证 Agent（主代理） | 判断完成质量 |
| **触发准确性** | 客观规则 | 系统 | 从执行日志判断 |
| **离线能力** | 客观规则 | 系统 | 从日志统计网络调用 |
| **资源效率** | 客观规则 | 系统 | Docker stats 采集 |
...
**由验证 Agent（主代理）自己评分**，评分标准已包含在 system prompt 中。
...
```python
{
    "task": "任务描述",
    "raw_score": 4,           # 1-5 分
    "converted_score": 75,    # 转换为 0-100 分：(raw_score - 1) * 25
    "reason": "评估理由",
    "skill_used": "skill-name",
    "correct_skill_used": True
}
...
```python
def calculate_objective_scores(task_results: list, metrics: dict) -> dict:
    """基于客观规则计算分数"""
    
    # 触发准确性（客观）
    correct_triggers = sum(1 for r in task_results if r.get("correct_skill_used"))
    trigger_score = (correct_triggers / len(task_results)) * 100
    
    # 离线能力（客观）
    blocked_calls = metrics.get("blocked_network_calls", 0)
    if blocked_calls == 0:
        offline_score = 100
...
def calculate_overall_score(
    completion_score: float,  # 验证 Agent 评估
...
验证 Agent 在评分后，生成**优点/缺点/改进建议**（包含在评估报告中）：
...
**Phase 1（当前）**：本地文件存储
**Phase 2（后续迭代）**：私有 Docker 仓库

```
镜像目录：settings.SKILL_IMAGES_DIR (如 D:\docker_volume\mutil-user-agent\skill-images)
                          ↓
镜像文件：skill-runtime-v1.0.tar → v1.1.tar → v1.2.tar → ...
                          ↑              ↑       ↑
...
class LocalFileImageBackend(ImageBackend):
    ...
    def list_versions(self) -> list[str]:
        ...
        versions = [f.stem.replace(f"{self.prefix}-", "") for f in tar_files]
...
### 4.4 私有仓库实现（Phase 2 预留）

```python
class RegistryImageBackend(ImageBackend):
    """私有 Docker 仓库后端（Phase 2）"""
    
    def __init__(self):
        self.registry_url = settings.DOCKER_REGISTRY_URL
        self.prefix = f"{self.registry_url}/skill-runtime"
...
通过对比 Agent 执行前后的命令历史，记录 Agent 执行了哪些安装操作：
...
def extract_dependencies_from_commands(commands: list[str]) -> dict:
    ...
    dependencies = {
        ...
        "downloaded": [],
        "other_install": [],
        "raw_commands": commands  # 保留原始命令供审计
    }
