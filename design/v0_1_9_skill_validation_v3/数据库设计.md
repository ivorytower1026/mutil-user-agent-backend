...
CREATE TABLE skills (
    ...
    trigger_accuracy_score INTEGER,
    offline_capability_score INTEGER,
...
CHECK (
    (usability_score IS NULL OR (usability_score >= 0 AND usability_score <= 100)) AND
...
| 字段 | 范围 | 权重 | 说明 |
|------|------|------|------|
| `usability_score` | 0-100 | 25% | 易用性评分 |
| `trigger_accuracy_score` | 0-100 | 30% | 触发准确性评分 |
| `offline_capability_score` | 0-100 | 25% | 离线能力评分 |
| `resource_efficiency_score` | 0-100 | 20% | 资源效率评分 |
| `validation_score` | 0-100 | - | 总体评分（加权平均） |
